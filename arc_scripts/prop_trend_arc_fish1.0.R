...
bio_fish1.0 <- bio_fish[1:bio_fish_index1,] #749,622
bio_fish1.0_combined <- add_mdat(bio_fish1.0, bio_all_mdat)
min_year = 1961
max_year = 2016
bio_fish1.0_pprepped <- prop_prep_ex(bio_fish1.0_combined, min_year, max_year)
write_rds(bio_fish1.0_pprepped, "data/pprepped/bio_fish1.0_pprepped.rds") #1970 to 2012 --> 1961 to 2016
bio_fish1.0_filled <- tibble()
for(i in 1:nrow(bio_fish1.0_pprepped)) {
  filled_row <- fill_missing(bio_fish1.0_pprepped[i,], i, "fish1.0")
  if(!is.null(filled_row)) {
    bio_fish1.0_filled <- rbind(bio_fish1.0_filled, filled_row)
  }
}
write_rds(bio_fish1.0_filled, "data/pfilled/bio_fish1.0_filled.rds")
