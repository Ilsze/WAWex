...
create_utility_visualizations <- function(data, 
  ...
  # 1.2 Create NC utility plots for FAERE
  create_nc_utility_plots_faere(data, output_dir, subdirectory = "faere", "NC_utility")
  create_nc_utility_plots_faere(data, output_dir, subdirectory = "faere", "NC_u_conc")  
  create_nc_utility_plots_faere(data, output_dir, subdirectory = "faere", "NC_u_conv")
  
  # # 2. Create WR utility plots  
  # create_wr_utility_plots(data, output_dir)
  #   
  # # 3. Prepare data for net series
  # extended_data_for_net <- prepare_data_for_net_series(data, pass_number, output_dir)
  # 
...
combine_utility_visualizations <- function(output_dir = "visualizations") {
  ...
  create_six_panel_population_display(output_dir = output_dir, 
                                        save_dir = paste0(pass_number, "/welfare_results/six_figure_displays"))
}
